...
/*
	Plugin Name: simply-picasaweb
	Plugin URI: http://rawlinson.us/blog/articles/picasaweb-wordpress-plugin/
	Description: Display thumbnails from one of your public picasaweb albums (or all albums).
...
class WP_PicasaWeb
{
		...
		function setArguments($args){
			...
			if (is_array($args)){
				...
				if(isset($args['linkToSourceImage']))
				{
					$p['linkToSourceImage']=$this->trueOrFalse($args['linkToSourceImage']);
				}
			} else {
				$p['url'] = $args;
			}
	...
	function display($args){
		...
		if($images = WP_PicasaWeb::loadFeed($p['url'])){
			...
			foreach ($images as $image) {
				...
				if($p['linkToAlbum']){
					$imgLink = $image['album_link'];
				} else if ($p['linkToSourceImage']){
					$imgLink = $image['img_src_link'];
				}
						//WP_PicasaWeb::debug($image);
	...
	function loadFeed($url){
		...
		// this regular expression matching is courtesy of Kiroro at http://kiroro.prophp.org/blog/
			...
			$items[] = array(
						...
						'img_src_link'=>$item['media:content']
					);
		}
